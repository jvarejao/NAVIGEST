...
public partial class NovaHoraView : ContentView
{
    ...
    private Action<HoraColaborador?> _onClose;

    public NovaHoraView(HoraColaborador hora, List<Colaborador> colaboradores, Action<HoraColaborador?> onClose)
    {
        InitializeComponent();
    ...
    private async void OnSelecionarTipoRegistoTapped(object sender, EventArgs e)
    {
        ...
        var options = new List<string> { AppResources.Hours_Work, AppResources.Hours_Absence };
        var popup = new GenericPickerPopup(AppResources.Hours_RecordType, options);
    ...
    private async void OnSelecionarMotivoAusenciaTapped(object sender, EventArgs e)
    {
        ...
        var options = _absenceTypes.Select(a => a.Description).ToList();
        var popup = new GenericPickerPopup(AppResources.Hours_SelectReason, options);
        var result = await page.ShowPopupAsync(popup);
        
        if (result is string action && !string.IsNullOrEmpty(action))
        {
    ...
    private async void OnSelecionarColaboradorClicked(object sender, EventArgs e)
    {
        ...
        var options = _colaboradores.Select(c => c.Nome).ToList();
        var popup = new GenericPickerPopup(AppResources.Hours_SelectCollaborator, options);
        var result = await page.ShowPopupAsync(popup);
    ...
    private async void OnGuardarClicked(object sender, EventArgs e)
    {
        ...
        if (TipoRegistoLabel.Text == AppResources.Hours_Work)
        {
            ...
            _hora.Observacoes = ObservacoesEditor.Text;
        }
        else
        {
