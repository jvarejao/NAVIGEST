...
namespace NAVIGEST.iOS.Behaviors;
...
/// <summary>
/// Behavior para Entries: altera cor do underline conforme foco e valida��o.
/// Android: usa BackgroundTintList.
/// Windows: simula underline via BorderBrush + BorderThickness (0,0,0,2) no TextBox nativo.
/// Outras plataformas: ajusta apenas TextColor (n�o h� underline nativo).
/// Estados: Foco -> Azul (#2563EB), V�lido -> Verde (#22C55E), Inv�lido -> Vermelho (#EF4444), Neutro -> Placeholder cinza.
/// </summary>
    ...
    private void UpdatePlatformUnderline(Color color)
    {
        ...
        try
        {
            if (_entry?.Handler?.PlatformView is AndroidX.AppCompat.Widget.AppCompatEditText native)
            {
                ...
                native.BackgroundTintList = Android.Content.Res.ColorStateList.ValueOf(aColor);
