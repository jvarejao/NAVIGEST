            ...
            try
            {
                ...
                if (sessionUser.CompanyLogo is null || sessionUser.CompanyLogo.Length == 0)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(logoBase64))
                    {
                        ...
                        {
                            Debug.WriteLine($"[MainYahPageViewModel] Invalid company logo base64: {ex.Message}");
                        }
                    }
                }
                ...
                CompanyName = string.IsNullOrWhiteSpace(sessionUser.CompanyName) ? "" : sessionUser.CompanyName;
                IsAdminUnlocked = string.Equals(sessionUser.Role, "ADMIN", StringComparison.OrdinalIgnoreCase);

                Debug.WriteLine($"[MainYahPageViewModel] Session refreshed. User='{UserName}', Role='{UserRole}', Company='{CompanyName}', LogoBytes={(sessionUser.CompanyLogo?.Length ?? 0)}");
            }
            catch (Exception ex) { TratarErro(ex); }
        }
