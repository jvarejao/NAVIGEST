    ...
    public async Task LoadAsync(bool force = false)
    {
        ...
        try
        {
            try
            {
                ...
                await AppShell.DisplayToastAsync($"OrderInfo: {count} linhas. Exemplo: {sampleNo ?? "-"} / {sampleCust ?? "-"}", NAVIGEST.iOS.ToastTipo.Info, 2000);
            }
            catch (Exception exProbe)
            {
                Debug.WriteLine($"[ServicePage] Debug probe failed: {exProbe}");
                NAVIGEST.iOS.GlobalErro.TratarErro(exProbe);
                await AppShell.DisplayToastAsync("Erro a sondar OrderInfo (ver Output).", NAVIGEST.iOS.ToastTipo.Erro, 2500);
            }
            ...
            try
            {
                ...
                await AppShell.DisplayToastAsync($"Serviços carregados: {list?.Count ?? 0}", NAVIGEST.iOS.ToastTipo.Info, 1400);
            }
            ...
            catch (Exception exLoad)
            {
                ...
                NAVIGEST.iOS.GlobalErro.TratarErro(exLoad);
                await AppShell.DisplayToastAsync("Erro ao carregar serviços (ver Output).", NAVIGEST.iOS.ToastTipo.Erro, 2000);
        ...
        catch (Exception ex)
        {
            ...
            await NAVIGEST.iOS.AppShell.DisplayToastAsync("Erro ao carregar serviços.", NAVIGEST.iOS.ToastTipo.Erro, 2000);
        }
        finally
        {
            IsBusy = false;
