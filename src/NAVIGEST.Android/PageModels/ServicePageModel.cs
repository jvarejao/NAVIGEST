    ...
    public async Task LoadAsync(bool force = false)
    {
        ...
        try
        {
            try
            {
                ...
                await AppShell.DisplayToastAsync($"OrderInfo: {count} linhas. Exemplo: {sampleNo ?? "-"} / {sampleCust ?? "-"}", NAVIGEST.Android.ToastTipo.Info, 2000);
            }
            catch (Exception exProbe)
            {
                Debug.WriteLine($"[ServicePage] Debug probe failed: {exProbe}");
                NAVIGEST.Android.GlobalErro.TratarErro(exProbe);
                await AppShell.DisplayToastAsync("Erro a sondar OrderInfo (ver Output).", NAVIGEST.Android.ToastTipo.Erro, 2500);
            }
            ...
            try
            {
                ...
                await AppShell.DisplayToastAsync($"Serviços carregados: {list?.Count ?? 0}", NAVIGEST.Android.ToastTipo.Info, 1400);
            }
            ...
            catch (Exception exLoad)
            {
                ...
                NAVIGEST.Android.GlobalErro.TratarErro(exLoad);
                await AppShell.DisplayToastAsync("Erro ao carregar serviços (ver Output).", NAVIGEST.Android.ToastTipo.Erro, 2000);
        ...
        catch (Exception ex)
        {
            ...
            await NAVIGEST.Android.AppShell.DisplayToastAsync("Erro ao carregar serviços.", NAVIGEST.Android.ToastTipo.Erro, 2000);
        }
        finally
        {
            IsBusy = false;
