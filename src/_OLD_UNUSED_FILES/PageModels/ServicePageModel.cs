    ...
    public async Task LoadAsync(bool force = false)
    {
        ...
        try
        {
            try
            {
                ...
                await AppShell.DisplayToastAsync($"OrderInfo: {count} linhas. Exemplo: {sampleNo ?? "-"} / {sampleCust ?? "-"}", AppLoginMaui.ToastTipo.Info, 2000);
            }
            catch (Exception exProbe)
            {
                Debug.WriteLine($"[ServicePage] Debug probe failed: {exProbe}");
                AppLoginMaui.GlobalErro.TratarErro(exProbe);
                await AppShell.DisplayToastAsync("Erro a sondar OrderInfo (ver Output).", AppLoginMaui.ToastTipo.Erro, 2500);
            }
            ...
            try
            {
                ...
                await AppShell.DisplayToastAsync($"Serviços carregados: {list?.Count ?? 0}", AppLoginMaui.ToastTipo.Info, 1400);
            }
            ...
            catch (Exception exLoad)
            {
                ...
                AppLoginMaui.GlobalErro.TratarErro(exLoad);
                await AppShell.DisplayToastAsync("Erro ao carregar serviços (ver Output).", AppLoginMaui.ToastTipo.Erro, 2000);
        ...
        catch (Exception ex)
        {
            ...
            AppLoginMaui.GlobalErro.TratarErro(ex);
            await AppLoginMaui.AppShell.DisplayToastAsync("Erro ao carregar serviços.", AppLoginMaui.ToastTipo.Erro, 2000);
        }
        finally
        {
            IsBusy = false;
